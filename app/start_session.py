...
async def start_session(
    *,
    ...
    with tt.Session.start(session_config) as session:
        skeleton = Skeleton(session)
        with tt.mapping_lookup(check=config.check_mapping_lookups):
            create_and_join_tables(skeleton)
            create_cubes(skeleton)
        await load_tables(skeleton, config=config, http_client=http_client)
        yield skeleton
